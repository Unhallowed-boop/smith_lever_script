window.loadLeverJobs = function (options) {
  ...
  function createJobs(responseData) {
    ...
    for(var i = 0; i < groupedPostings.length; i++) {
      ...
      if (haveDepartments) {
        content += '<div class="lever-department" data-department="' + groupedPostings[i].departmentTitle + '"><h3 class="lever-department-title">' + sanitizeForHTML(groupedPostings[i].departmentTitle) + '</h3>';
      };
